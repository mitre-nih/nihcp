...
// TODO make sure only appropriate owners and roles access this.
...
} else if($request_guid && get_entity($request_guid)) {
	$request = get_entity($request_guid);
	$content = elgg_view('nihcp_credit_allocation/allocations/allocations_for_request', array('request' => $request, 'review_mode' => $review_mode));
} else {
